package s3_test

import (
	"frankie060392/rest-api-clean-arch/bootstrap"
	"frankie060392/rest-api-clean-arch/pkg/aws/s3"
	"strings"
	"testing"

	"github.com/stretchr/testify/assert"
)

func TestS3(t *testing.T) {
	base64Image := "data:image/png;base64,"

	keyName := "test.png"

	b64data := base64Image[strings.IndexByte(base64Image, ',')+1:]

	config := bootstrap.LoadConfig(".")

	awsSession := s3.InitAWSConnection(config.AwsAccessKeyId, config.AwsSecretKey, "", config.AwsS3Region)
	err := s3.S3UploadBase64(awsSession, config.AwsS3Bucket, b64data, keyName)

	assert := assert.New(t)
	assert.Nil(err, "upload base64 image to s3")
}
